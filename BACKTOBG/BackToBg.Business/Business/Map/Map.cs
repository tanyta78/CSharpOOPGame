...
namespace BackToBg.Core.Business.Map
{
    public class Map : IMap
    {
        ...
        public int Size { get; } = Constants.DefaultMapSize;
        ...
        public void DrawMap()
        {
            ...
            for (var row = Math.Max(0, playerInfo.row - consoleHeight / 2);
        ...
        public void GenerateMap()
        {
            ...
            for (var i = 0; i < this.Size; i++)
                this.map[i] = new string(Constants.RoadChar, this.Size).ToCharArray();
            ...
            foreach (var building in this.buildings)
            {
                ...
                var x = info.row;
                var y = info.col;
                for (var row = x; row < Math.Min(x + figure.Length, this.Size - 1); row++)
                for (var col = y; col < Math.Min(y + figure[0].Length, this.Size - 1); col++)
            ...
            this.map[0] = new string(border, this.Size).ToCharArray();
            for (var i = 1; i < this.Size - 1; i++)
            {
                this.map[i][0] = border;
                this.map[i][this.map[0].Length - 1] = border;
            }
            this.map[this.Size - 1] = new string(border, this.Size).ToCharArray();
        }
    }
}